...
2. En el **SQL Editor**, ejecutá `supabase/migrations/0001_init.sql` (tablas `formaciones`, `roles`, `historial`, RLS, Realtime y seed de 23 formaciones).
...
- **Edición automática del admin**: al tocar una fecha/estado se guarda solo (optimista → IndexedDB → Sync a Supabase). Sin conexión queda encolado y sincroniza al reconectar.
...
- PWA instalable, scroll oculto, fondo con foto `trenes.jpg` + capa oscura.
...
src/
  ...
  index.css                 Tokens @theme, fondo (imagen + capa oscura),
                            scroll oculto
  hooks/
...
1. **Admin edita** una tarjeta → `FormationCard.onChange` → `aplicarCambio` (`useFormaciones.ts`).
...
- `public.formaciones`: `id`, `formacion` (único), `anteultima` (date), `ultima` (date), `estado` (`activa | limpieza | reparacion | fuera-servicio`), `updated_at`. `dias` y `sem` NO se guardan: se calculan en el cliente.
